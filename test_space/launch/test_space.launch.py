...
def launch_robot_state_publisher(context, file_name_var):
    test_space_directory_path = get_package_share_directory('test_space')
    file_name = context.perform_substitution(file_name_var)
    print(F"file_name: {file_name}")
    xacro_file_path = os.path.join(test_space_directory_path, 'urdf', file_name)
...
def generate_launch_description():
    launch_args = [DeclareLaunchArgument(
        'urdf', default_value='base.urdf', description='URDF file name')]
    ...
    rviz_node = launch_ros.actions.Node(
        ...
        arguments=['-d', os.path.join(test_space_directory_path, 'rviz', 'config.rviz')],
    )
