...
class SaySomethingActionServer : public rclcpp::Node {
 public:
  ...
  /**
   * @brief A destructor for custom_behaviors::SaySomethingServer
   */
 ...
 private:
  ...
  void execute(const std::shared_ptr<GoalHandleSaySomething> goal_handle) {
    ...
    RCLCPP_INFO(this->get_logger(), "[SaySomething2!!!] [x: %4.3f, y: %4.3f, z-rad: %4.3f] %s",
...
RCLCPP_COMPONENTS_REGISTER_NODE(custom_behaviors::SaySomethingActionServer)